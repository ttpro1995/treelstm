...
function TreeLSTMSentiment:__init(config)
    ...
    elseif self.structure == 'rnn' then
        self.treelstm = treelstm.BinaryTreeRNN(treelstm_config)
    else
...
function TreeLSTMSentiment:new_sentiment_module()
    ...
    sentiment_module:add(nn.Linear(self.mem_dim, self.num_classes)):add(nn.LogSoftMax())
    ...
    for i = 1, dataset.size, self.batch_size do
        ...
        local feval = function(x)
            ...
            for j = 1, batch_size do
                ...
                local input_grad
                --vinh replace adhoc
                if self.structure == 'rnn' then
                    input_grad = self.treelstm:backward(tree, inputs, zeros)
                else input_grad = self.treelstm:backward(tree, inputs, { zeros, zeros })
                end
